...
//       originalTotal += price * product.quantity;
//     });

//     let couponEligibleTotal = 0;

//     cartArray.forEach(product => {
//       if (product.couponSelected) {
//         couponEligibleTotal +=
//           (product.couponDiscount || 0) * product.quantity;
//       }
//     });

//     const walletUsed = Math.min(walletBalance, couponEligibleTotal);
//     const walletRemaining = walletBalance - walletUsed;
//     const grandTotal = originalTotal - walletUsed;
...
//       {/* ---------------- ORDER SUMMARY ---------------- */}
//       <div className="max-w-[360px] w-full bg-gray-100 p-5 ml-8">

//         <h2 className="text-xl font-medium">Order Summary</h2>

//         {/* Address */}
//         <p className="text-sm font-medium uppercase mt-5">
//           Delivery Address
//         </p>
...
//               <p
//                 onClick={() => navigate('/add-address')}
//                 className="p-2 text-primary cursor-pointer hover:bg-gray-100"
//               >
//                 Add address
//               </p>
//             </div>
...
//             <span>Price</span>
//             <span>{currency}{cartTotals.originalTotal.toFixed(2)}</span>
//           </div>

//           <div className="flex justify-between">
//             <span>Wallet Used</span>
//             <span className="text-green-600">
//               -{currency}{cartTotals.walletUsed.toFixed(2)}
//             </span>
//           </div>

//           <div className="flex justify-between">
//             <span>Wallet Remaining</span>
//             <span>{currency}{cartTotals.walletRemaining.toFixed(2)}</span>
//           </div>

//           <div className="flex justify-between text-lg font-semibold mt-2">
//             <span>Total Amount</span>
//             <span>{currency}{cartTotals.grandTotal.toFixed(2)}</span>
//           </div>
//         </div>
...
//           onClick={placeOrder}
//           disabled={placingOrder}
//           className="w-full py-3 mt-6 bg-primary text-white disabled:opacity-50"
//         >
//           {placingOrder
//             ? "Processing..."
//             : paymentOption === "COD"
...
//     // 1️⃣ Calculate original cart total (NO wallet deduction)
//     cartArray.forEach(product => {
//       const price = product.offerPrice || 0;
//       originalTotal += price * product.quantity;
//     });

//     // 2️⃣ Calculate coupon eligible amount
//     let couponEligibleTotal = 0;
...
//     // 3️⃣ Wallet usage logic (limited by coupon selected items)
//     const walletUsed = Math.min(walletBalance, couponEligibleTotal);

//     const walletRemaining = walletBalance - walletUsed;

//     // 4️⃣ Final payable
//     const grandTotal = Math.max(originalTotal - walletUsed, 0);
...
//             {/* ORIGINAL line total (never changes) */}
//             <p className="text-center">
//               {currency}
//               {(product.offerPrice * product.quantity).toFixed(2)}
//             </p>
...
const Cart = () => {
  ...
  const calculateWalletSummary = () => {
    ...
    cartArray.forEach(product => {
      ...
      if (product.couponSelected && remainingWallet > 0) {
        const maxCoupon = couponValue * quantity;
        const deduction = Math.min(maxCoupon, remainingWallet);
        totalWalletUsed += deduction;
        remainingWallet -= deduction;
      }
    });

    const grandTotal = totalPrice - totalWalletUsed;
  ...
  const { totalPrice, grandTotal, walletUsed, walletRemaining } = calculateWalletSummary();
  ...
  return (
    <div className="flex flex-col md:flex-row mt-16 px-10">
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70 ml-0 md:ml-8">
        ...
        <div className="text-gray-500 mt-4 space-y-2">
          <p className="flex justify-between">
            <span>Price</span><span>{currency}{totalPrice.toFixed(2)}</span>
          </p>
          <p className="flex justify-between">
            <span>Wallet Used</span><span className="text-green-600">-{currency}{walletUsed.toFixed(2)}</span>
          </p>
